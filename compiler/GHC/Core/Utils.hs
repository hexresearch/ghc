...
(See Note [Eta reduction is right-to-left] about the half-way attempt to
transform to `\x. e x`.)
...
Note [Eta reduction is right-to-left]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
As Note [Eta reduction soundness] says, eta reduction transforms
`\x y. e x y` to `e`, completely. By contrast, the half-way transformation to
`\x. e x` is *not* quite eta-reduction, at least not on `\x y. e x y`:
Eta-expansion on it would yield `\y. (\x. e x) y`. It is however eta-reduction
on the sub-expression `\y. e x y` in the context `\x. _`.

Nevertheless, it seems like a useful and more generally applicable
transformation to attempt. Can we adopt the soundness criteria from eta
reduction?
No. At least not in the following form:

 A. "It is sound to half-way-eta-reduce n arguments as long as n does
     not exceed the `exprArity` of `e`."
    Wrong: Consider `e = \x. bot` (arity 1). Then we might be tempted to
    half-way-eta-reduce 1 arg of `\x y. e x y` to `\x. e x`, but the latter
    diverges when applied to one argument whereas the former does not.
    We really eta-reduce `\y. (e x) y` and we have to look at the arity of `e
    x`, which is 0.

 S. "It is sound to half-way-eta-reduce n arguments in an evaluation context in
     which all calls happen with *exactly* n arguments."
    Wrong: Consider
      ``g c = Just (c 1 `seq` c 2 `seq` 42)``
    Then we might be tempted to half-way-eta-reduce the argument of
    `g (\x y. e x y)` to `g (\x. e x)`, but the latter diverges for
    `e = \x. bot`.
    We really eta-reduce `\y. (e x) y` in the context `\x. _` and that
    context will eat one argument, meaning that all calls to
    `\y. (e x) y` really happen with n-1 arguments.

Post morten: Eta reduction right-to-left on lambda binders/args; inner
eta redexes are reduced first: `\x _y. e x y_` ===> `_\x. e x_` ===> `e`
But both arity and strictness info tell us only something about the *first* n
lambda binders/args applied to `e`, so left-to-right. We can only exploit arity
and strictness when the "last n binders" coincide with the "first n binders",
but then we just do eta-reduction of the whole group.
