...
mkRhsStop ty bndr_dmd = Stop ty RhsCtxt rhs_sd
  where
    _ :* rhs_sd = bndr_dmd
...
contEvalContext k = case k of
  ...
  StrictArg{sc_fun=fun_info} -> unpack_dmd (head (ai_dmds fun_info))
  StrictBind{sc_bndr=bndr}   -> unpack_dmd (idDemandInfo bndr)
  Select{}                   -> topSubDmd
