...
exprIsNewtypeDict_maybe :: InScopeEnv -> CoreExpr -> Maybe CoreExpr
...
exprIsNewtypeDict_maybe (in_scope, id_unf) e =
  -- pprTraceWith "exprIsNewtypeDict_maybe" (\answer -> ppr e $$ ppr answer) $
  go in_scope e []
  where
    go in_scope (App f a)  args = go in_scope f (a:args)

    go in_scope (Var fun) args     -- See Note [Unfolding DFuns]
      ...
      , isNewDataCon con
      , bndrs `equalLength` args   -- See Note [DFun arity check]
      , [op_app] <- filter isRuntimeArg df_args
      -- op_app=$cop[] @a $da
      -- => subst=[a:->Int,$da:->$fCInt]. Then subst(op_app)=$cop[] @Int $fCInt
      = Just (substExpr (mkDFunArgsSubst in_scope fun unfolding args) op_app)

      | Just rhs <- expandUnfolding_maybe unfolding
           -- Try looking through CoreUnfoldings. See the case in
           -- exprIsConApp_maybe above
      , let in_scope' = extendInScopeUnfoldingFVs in_scope fun unfolding
      = go in_scope' rhs args
      where
        unfolding = id_unf fun
