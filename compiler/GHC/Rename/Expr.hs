...
-- XXX JB do \cases
rnExpr (HsLamCase x lcVariant matches)
  = do { (matches', fvs_ms) <- rnMatchGroup CaseAlt rnLExpr matches
       ; return (HsLamCase x lcVariant matches', fvs_ms) }
...
-- XXX JB do \cases
rnCmd (HsCmdLamCase x lcVariant matches)
  = do { (new_matches, ms_fvs) <- rnMatchGroup (ArrowMatchCtxt ArrowCaseAlt) rnLCmd matches
       ; return (HsCmdLamCase x lcVariant new_matches, ms_fvs) }
