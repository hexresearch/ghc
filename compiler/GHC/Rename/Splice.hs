...
module GHC.Rename.Splice (
        rnTopSpliceDecls,
        rnSpliceType, rnTypedSpliceExpr, rnUntypedSpliceExpr, rnSplicePat, rnSpliceDecl,
...
rnUntypedSpliceGen :: (HsUntypedSplice GhcRn -> RnM (a, FreeVars))
                   ...
                   -> (HsUntypedSplice GhcRn -> (PendingRnSplice, a))
...
runRnSplice flavour run_meta ppr_res splice
  = do { hooks <- hsc_hooks <$> getTopEnv
       ...
       ; let the_expr = case splice' of
                HsUntypedSpliceExpr _ _ e      ->  e
                HsQuasiQuote (_, q) str -> undefined -- ROMES:TODO mkQuasiQuoteExpr flavour q qs str
...
makePending :: UntypedSpliceFlavour
            -> HsUntypedSplice GhcRn
            -> PendingRnSplice
makePending flavour (HsUntypedSpliceExpr (_, n) _ e)
  = PendingRnSplice flavour n e
makePending flavour (HsQuasiQuote (n, quoter) quote)
  = PendingRnSplice flavour n undefined -- ROMES:TODO: (mkQuasiQuoteExpr flavour quoter q_span quote)
...
rnUntypedSplice (HsUntypedSpliceExpr (annCo, splice_name) hasParen expr)
  = do  { loc  <- getSrcSpanM
        ; n' <- newLocalBndrRn (L (noAnnSrcSpan loc) splice_name)
        ; (expr', fvs) <- rnLExpr expr
        ; return (HsUntypedSpliceExpr (annCo, n') hasParen expr', fvs) }

rnUntypedSplice (HsQuasiQuote (splice_name, quoter) quote)
  = do  { loc  <- getSrcSpanM
        ...
        ; return (HsQuasiQuote (splice_name', quoter') quote, unitFV quoter') }

---------------------
rnTypedSpliceExpr :: (EpAnnCO, EpAnn [AddEpAnn], IdP GhcPs) -- XTypedSplice ROMES:TODO: Can I Use XTypedSplice GhcPs directly?
                  -> LHsExpr GhcPs                          -- Typed splice expression
                  -> RnM (HsExpr GhcRn, FreeVars)
rnTypedSpliceExpr (annCo, annL, splice_name) expr
  = addErrCtxt (hang (text "In the typed splice:") 2 ({- ROMES:TODO: Correct typed ppr with $$ -} ppr expr)) $ do
    ...
    ; case stage of
        ...
        _ -> do { extEnabled <- xoptM LangExt.TemplateHaskell
                ...
                ; traceRn "rnTypedSpliceExpr: typed expression splice" empty
  ...
  where
    ...
    rnTypedSplice =
      do { loc <- getSrcSpanM
         ; n' <- newLocalBndrRn (L (noAnnSrcSpan loc) splice_name)
         ; (expr', fvs) <- rnLExpr expr
         ; return (HsTypedSplice (annCo, annL, n') expr', fvs) }
...
rnUntypedSpliceExpr splice
  ...
  where
    pend_expr_splice :: HsUntypedSplice GhcRn -> (PendingRnSplice, HsExpr GhcRn)
    pend_expr_splice rn_splice
        = (makePending UntypedExpSplice rn_splice, HsUntypedSplice undefined rn_splice) -- ROMES:TODO: ... can't make HsUntypedSpliceResult (HsExpr GhcRn)
    ...
    run_expr_splice rn_splice
      = do { traceRn "rnUntypedSpliceExpr: untyped expression splice" empty
           ...
           ; let e =  flip HsUntypedSplice rn_splice
                    . HsUntypedSpliceResult (ThModFinalizers mod_finalizers)
                        <$> lexpr3
           ; return (gHsPar e, fvs)
           }
  ...
  where
    pend_type_splice rn_splice
       = ( makePending UntypedTypeSplice rn_splice
         , HsSpliceTy undefined rn_splice) -- ROMES:TODO: Big design issue...
...
rnSplicePat splice
  ...
  where
    pend_pat_splice :: HsUntypedSplice GhcRn ->
                       (PendingRnSplice, Either b (HsUntypedSplice GhcRn))
    pend_pat_splice rn_splice
      = (makePending UntypedPatSplice rn_splice
        , Right rn_splice) -- ROMES:TODO: ...

    run_pat_splice :: HsUntypedSplice GhcRn ->
                      RnM (Either ( HsUntypedSplice GhcRn
                                  , HsUntypedSpliceResult (Pat GhcPs) ) -- ROMES:TODO: explain in note..
    ...
    run_pat_splice rn_splice
      = do { traceRn "rnSplicePat: untyped pattern splice" empty
           ; (pat, mod_finalizers) <-
             ...
             -- See Note [Delaying modFinalizers in untyped splices].
           ; let p = HsUntypedSpliceResult (ThModFinalizers mod_finalizers) (gParPat pat)
