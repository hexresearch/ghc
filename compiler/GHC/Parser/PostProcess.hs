...
(checkExpBlockArguments, checkCmdBlockArguments) = (checkExpr, checkCmd)
  where
    ...
    checkExpr expr = case unLoc expr of
      ...
      HsLamCase _ lcKind _ -> check (PsErrLambdaCaseInFunAppExpr lcKind) expr
    ...
    checkCmd cmd = case unLoc cmd of
      ...
      HsCmdLamCase _ lcKind _ -> check (PsErrLambdaCaseCmdInFunAppCmd lcKind) cmd
...
class (b ~ (Body b) GhcPs, AnnoBody b) => DisambECP b where
  ...
  mkHsLamCasePV :: SrcSpan -> LamCaseKind
...
instance DisambECP (HsCmd GhcPs) where
  ...
  mkHsLamCasePV l lcKind (L lm m) anns = do
    cs <- getCommentsFor l
    let mg = mkMatchGroup FromSource (L lm m)
    return $ L (noAnnSrcSpan l) (HsCmdLamCase (EpAnn (spanAsAnchor l) anns cs) lcKind mg)
...
instance DisambECP (HsExpr GhcPs) where
  ...
  mkHsLamCasePV l lcKind (L lm m) anns = do
    cs <- getCommentsFor l
    let mg = mkMatchGroup FromSource (L lm m)
    return $ L (noAnnSrcSpan l) (HsLamCase (EpAnn (spanAsAnchor l) anns cs) lcKind mg)
...
instance DisambECP (PatBuilder GhcPs) where
  ...
  mkHsLamCasePV l lcKind _ _ = addFatalError $ mkPlainErrorMsgEnvelope l (PsErrLambdaCaseInPat lcKind)
