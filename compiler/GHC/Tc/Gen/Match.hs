...
tcMatchLambda herald match_ctxt match res_ty
  = matchExpectedFunTys herald GenSigCtxt match' res_ty $ \ bndrs rhs_ty ->
    tcMatches match_ctxt bndrs rhs_ty match
  where
    match' | isEmptyMatchGroup match = trivial_match_group
           | otherwise               = match
    trivial_match = mkMatch LambdaExpr [mkVisMatchPat nlWildPatName] (L noSrcSpanA expr) emptyLocalBinds
...
tcMatches ctxt bndrs rhs_ty (MG { mg_alts = L l matches
  ...
  = do { tcEmitBindingUsage bottomUE
       ...
       ; return (MG { mg_alts = L l []
                    , mg_ext = MatchGroupTc bndrs' rhs_ty
                    , mg_origin = origin }) }
